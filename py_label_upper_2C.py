...
class UiMainWindow(QMainWindow, FormWindow):
    ...
    def emg_open(self):
        try:
            self.emg = Serial('COM'+str(self.PORT_EMG.text()), baudrate=250000)
            if self.emg.readable():
                line = self.emg.readline().decode() # 한 번만 readline 호출

                self.values = line.split(',')

                if len(self.values) >= 2:  # 최소 두 개의 채널 데이터가 있는지 확인
                    self.C1_value = int(self.values[0])
                    self.C2_value = int(self.values[1])
                    # 필요한 경우 C3_value, C4_value도 추가

                self.C1_f1 = [self.C1_value] * 10
                self.C2_f1 = [self.C2_value] * 10
                # self.C3_f1 = [int(self.emg.readline().decode().split(',')[2])] * 10
                # self.C4_f1 = [int(self.emg.readline().decode().split(',')[3])] * 10
    ...
    def draw_emg(self):
        try:
            self.PLOT.clear()
            if self.emg.readable():
                    self.value_List = self.emg.readline().decode().split(',')
                    print(self.value_List)
                    self.C1_value = int(self.value_List[0])
                    self.C2_value = int(self.value_List[1])
                ...
                # if len(self.C1_value) > 0:
                    self.update_data_C1(int(self.C1_value))
                    self.update_data_C2(int(self.C2_value))
                    ...
                    if self.isRecord:
                        if self.t_remain == 0:
                            if self.c_label == 0:
                                ...
                                self.t_remain = self.a_time * 50  # active time 설정
                                self.v_time = time.time() + self.a_time
                            else:
                                self.c_label = 0  # rest label 선택
                                self.t_remain = self.r_time * 50  # rest time 설정
                                self.v_time = time.time() + self.r_time

                        self.t_remain -= 1  # 시간 감소
                        self.csv_f.writerow([self.C1_sf4[-1], self.C2_sf4[-1], self.c_label,
